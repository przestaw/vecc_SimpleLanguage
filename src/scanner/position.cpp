...
Position::Position() : lineNo(1), symbolNo(0), namedSource(false) {}

Position::Position(std::string source)
    : lineNo(1), symbolNo(0), namedSource(true), sourceName(std::move(source)) {
}

Position::Position(unsigned line, unsigned symbol)
    : lineNo(line), symbolNo(symbol), namedSource(false) {}

Position::Position(unsigned line, unsigned symbol, std::string streamName)
    : lineNo(line), symbolNo(symbol), namedSource(true),
      sourceName(std::move(streamName)) {}

bool Position::operator==(const Position &rhs) const {
  return lineNo == rhs.lineNo && symbolNo == rhs.symbolNo
